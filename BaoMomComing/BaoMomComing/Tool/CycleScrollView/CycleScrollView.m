...
//        NSLog(@"next，当前页:%d",self.currentPageIndex);
        if (self.getCurrentPage) {
            self.getCurrentPage(self.currentPageIndex);
        }
        [self configContentViews];
    }
    if(contentOffsetX <= 0) {
...
//    }
    ...
    if (self.delegate && [self.delegate respondsToSelector:@selector(tapActionWithPageIndex:)]) {
        [self.delegate tapActionWithPageIndex:self.currentPageIndex];
    }
}
